...
$ambil = $koneksi->query("SELECT * FROM tbl_kaskeluar where substr(tanggal,1,4)= '$tahun' 
                        and substr(tanggal,6,2) = '$bulan' order by tbl_kaskeluar.tanggal");

$ambil2 = $koneksi->query("SELECT * FROM tbl_kaskeluar where substr(tanggal,6,2) = '$bulan'");
$tampil2 = $ambil2->fetch_assoc();
$format_bulan_2 = $tampil2['tanggal'];

$ambil4 = $koneksi->query("SELECT * FROM tbl_kaskeluar where substr(tanggal,1,4)= '$tahun' 
                        and substr(tanggal,6,2) = '$bulan' order by tbl_kaskeluar.tanggal");
...
if($jumlah_kas < 1){
    ...
    $ambil3= $koneksi->query ("SELECT SUM(total_harga) AS total FROM tbl_kaskeluar where substr(tanggal,6,2) = '$bulan' 
